...
def HomePage():
    ...
    st.markdown("<hr><p style='text-align: center;'>Built with by Datapsalm & Victoria</p>", unsafe_allow_html=True)
...
def GPACalculator():
    ...
    current_cgpa = st.number_input(f"Current CGPA (0 if fresher) [max {max_scale}]:", min_value=0.0, max_value=max_scale, step=0.01)
    ...
    if st.button("Calculate Updated CGPA"):
        ...
        st.markdown("---")
        st.subheader(f"Updated CGPA: {round(updated_cgpa, 2)}")

        # Show semester GPAs
        for sem in semester_data:
            sem_gpa = sem['total_points'] / sem['total_units'] if sem['total_units'] > 0 else 0
            st.markdown(f"- Semester {sem['semester']} GPA: {round(sem_gpa, 2)}")
        ...
        for sem in semester_data:
            ...
            sem_gpa = sem['total_points'] / sem['total_units'] if sem['total_units'] > 0 else 0
            result_txt.write(f"Semester GPA: {round(sem_gpa, 2)}\n")

        result_txt.write(f"\nUpdated CGPA: {round(updated_cgpa, 2)}\n")
...
def WhatDoINeed():
    ...
    current_cgpa = st.number_input("Current CGPA:", min_value=0.0, max_value=5.0, step=0.01)
    ...
    target_cgpa = st.number_input("Target CGPA:", min_value=0.0, max_value=5.0, step=0.01)
    ...
    if st.button("Calculate Required GPA"):
        ...
        # Equal split (current method)
        equal_gpa = points_remaining / total_future_credits
        
        # Scenario 1: First semester lower, second higher (+10% variation)
        sem1_lower = max(0, equal_gpa * 0.9)
        sem2_higher = min(5.0, (points_remaining - sem1_lower * sem1_credits) / sem2_credits)
        
        # Scenario 2: First semester higher, second lower (-10% variation)
        sem1_higher = min(5.0, equal_gpa * 1.1)
        sem2_lower = max(0, (points_remaining - sem1_higher * sem1_credits) / sem2_credits)
        
        st.markdown("---")
        st.subheader("Suggested GPA Plans for Next 2 Semesters")

        st.markdown(f"**Option 1 (Equal GPA Both Semesters):**")
        st.markdown(f"- Semester 1: {round(equal_gpa,2)}  |  Semester 2: {round(equal_gpa,2)}")

        st.markdown(f"**Option 2 (Semester 1 Lower, Semester 2 Higher):**")
        st.markdown(f"- Semester 1: {round(sem1_lower,2)}  |  Semester 2: {round(sem2_higher,2)}")

        st.markdown(f"**Option 3 (Semester 1 Higher, Semester 2 Lower):**")
        st.markdown(f"- Semester 1: {round(sem1_higher,2)}  |  Semester 2: {round(sem2_lower,2)}")
